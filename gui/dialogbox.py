...
class DialogBox(TextBox):
    ...
    def split_line_list(self, line):
        ...
        while( lib.get_text_size(
                ''.join(part['text'] for part in line[:color_part_index+1]),
                self.font)[0] < self.text_width ):
            color_part_index +=1
        ...
        while(lib.get_text_size(
                ''.join(part['text'] for part in line[:color_part_index])+line[color_part_index]['text'][:relative_letter_index],
                self.font)[0] < self.text_width ):
        ...
        if (any(' ' in k['text'] for k in line[:color_part_index])) or (' ' in line[color_part_index]['text'][:relative_letter_index+1]):
        ...
        left.append({'color':line[color_part_index]['color'],'text':line[color_part_index]['text'][:relative_letter_index]})
        right = line[color_part_index+1:]
        right.insert(0,{'color':line[color_part_index]['color'],'text':line[color_part_index]['text'][relative_letter_index:]})
    ...
    def process_text(self, text):
        ...
        while i < length:
            ...
            if not line:
                ...
                continue
            #print(line)
            line_w = lib.get_text_size(''.join([data['text'] for data in line]).rstrip(), self.font)[0]
    ...
    def set_text(self, text, color=None, text_color=None):
        if text == self.text and color in [None, self.color] and text_color in [None, self.text_color]:
            return
    ...
    def draw(self):
        ...
        for i in range(len(self.text_list)):
            ...
            self.text_surf = lib.render_text(
                ''.join(color_part['text'] for color_part in line), self.font, self.text_color)
