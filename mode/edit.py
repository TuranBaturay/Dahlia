...
class Edit(Mode):
    def __init__(self, app, display) -> None:
        ...
        pygame.draw.polygon(self.up_triangle,lib.cloud_white,[(15,10),(10,20),(20,20)])
    ...
    def init_gui(self):
        ...
        gui.Button(
            self.gui_list,
            self.main_panel.rect.right-80,10,50,30,"Save",
        ...
        self.tileset_label=gui.TextBox(
            self.gui_list,
            *selector.rect.topleft,selector.rect.w,30,"Tileset",
            color=lib.wet_blue,
            border_radius=10,
        ...
        gui.Button(
            self.gui_list,
            self.tileset_label.rect.right-30,self.tileset_label.rect.top,
            30,30,">",color=lib.wet_blue,border_radius=10,func=self.switch_tileset
        )
        page_button_panel = gui.Panel(
            self.gui_list,
            selector.rect.right-50,0,40,120,
            color = lib.dark_blue,
            border=2,border_radius=10,
            border_color=lib.wet_blue
        )
        ...
        self.inspector_button = gui.Button(
            self.gui_list,
            0,0,30,30,
            "i",border_radius=10,color=lib.dark_blue,border=2,border_color=lib.light_blue,
            func=lambda:[self.app.get_inspector().select_tile(self.hovered_layer_tile),self.app.set_mode("inspector")]
        )
        ...
        for y in range(2):
            for x in range(4):
                gui.Button(
                    self.gui_list,
                    selector.rect.x+25+70*x,selector.rect.y+20+70*y,
                    64,64,
                    color=[0,0,0,0],
                    uid="tile_button"
                )
        toggle =gui.Toggle(
            self.gui_list,
            selector.rect.x,selector.rect.bottom+20,
            360,30,"Collision",func=lambda value: self.toggle_collision(value),color=lib.wet_blue,border_radius=10
        )
        ...
        self.layer_selector_label =gui.TextBox(
            ...
            *self.layer_selector.rect.topleft,
            360,30,"LAYERS",border_radius=10,
            color=lib.wet_blue
        )
        self.add_layer_button =gui.Button(
            self.gui_list,
            self.layer_selector_label.rect.right-30,self.layer_selector.rect.top,
            30,30,"+",color=lib.dark_turquoise,
            border_radius=10,border=3,border_color=lib.wet_blue,
    ...
    def update_layer_selector(self):
        self.gui_list = [panel for  panel in self.gui_list if panel.uid not in ["layer_button","layer_label"]]
        self.layer_labels = []
        ...
        for i,layer in enumerate(reversed(self.app.level.layers)):
            ...
            label = gui.Button(
                self.gui_list,
                x_pos,y_pos,190,30,layer[0],
                color=lib.dark_turquoise if self.current_layer == layer[0] else lib.wet_blue,
                uid="layer_label",align="left",
                border_radius=10,
                func=lambda layer=layer:self.set_layer(layer[0]),
            )
            label.set_right_click_func(lambda layer=layer: 
                self.text_input.ask_input(
                    lambda text,layer=layer: [self.rename_layer(layer[0],text),self.update_layer_selector()] if text else None,
                    400,200,"Rename layer",25
                )
            )
            x_pos += 200
            swap_up =gui.Button(
                ...
                x_pos,y_pos,30,30,
                image=self.up_triangle,
                border_radius=10,color=lib.wet_blue,uid="layer_button",
            ...
            if i == max_len-1:swap_down.disable()
            x_pos += 35

            show_button = gui.Button(
                self.gui_list,
                x_pos,y_pos,
                30,30,border_radius=10,image=self.eye_open if layer[1].visible else self.eye_close,
                color=lib.wet_blue,uid="layer_button",

            )
            show_button.set_func(                
                lambda layer=layer,show_button=show_button:[
                    layer[1].toggle_visibility(),show_button.set_img(
            ...
            remove_button = gui.Button(
                self.gui_list,
                x_pos,y_pos,
                30,30,"x",color=lib.darker_red,border_radius=10
                ,uid="layer_button",
                func=lambda layer = layer: 
                    self.text_input.ask_input(
                        lambda text,layer=layer:self.remove_layer(layer[0]) if text and text.lower() in ["yes","y"] else None,
                        400,200,f"Remove '{layer[0]}' ? (yes/no)" ,5
                    )
            )
            if max_len == 1:
    ...
    def update_selector(self):
        tileset = lib.animated_tileset_cache if self.current_tileset == "animated" else lib.tileset_cache
        ...
        for x,tile_button in enumerate(lib.get_by_id(self.gui_list,"tile_button")):
            if x+8*self.pages[self.current_tileset] >= max_len:
                ...
                tile_button.set_func(lambda x=x:self.select_tile(x+8*self.pages["static"]))
            else:
                #print(x+8*self.pages["animated"],max_len)
                tile_button.set_img(lib.animated_tileset_get(x+8*self.pages["animated"],0))
                tile_button.set_func(lambda x=x:self.select_tile(x+8*self.pages["animated"]))
    ...
    def select_tile(self,index):
        ...
        if self.current_tileset == "animated":
            self.preview_tile.set(
                {"index":index,"collision":self.preview_tile.collision,"flip":self.preview_tile.flip,
                "animation_duration":[5]*(len(lib.animated_tileset_cache[index])-1)}
            )
            ...
            self.preview_tile_img = lib.tileset_get(self.preview_tile.index,self.preview_tile.flip)
    ...
    def update(self, dt, mouse, mouse_button, mouse_pressed):
        ...
        if not self.main_panel.rect.collidepoint(mouse):
            ...
            self.preview_border.topleft = (self.app.virtual_mouse[0]*64,self.app.virtual_mouse[1]*64)
            ...
            self.hovered_layer_tile = self.app.level.get(*self.app.virtual_mouse,self.current_layer)
            self.app.debugger.set("Tile",self.hovered_tile.format() if self.hovered_tile else None)
            pygame.draw.rect(
                self.display,
                lib.darker_red if any(i < 0 for i in self.app.virtual_mouse) else lib.cloud_white, 
            ...
            if self.hovered_layer_tile and (
                ...
                pygame.draw.rect(self.display,lib.light_blue,self.preview_tile.rect,3)
                if mouse_pressed[0]:
                    self.app.level.set(
        ...
        for panel in self.gui_list:
            ...
            if panel.visible and panel.uid not in ["tile_button"]:self.display.blit(panel.image,panel.rect)

        for x,tile_button in enumerate(lib.get_by_id(self.gui_list,"tile_button")):
            if self.selected_tiles[self.current_tileset] == x+8*self.pages[self.current_tileset] :
                anim_rect = tile_button.rect.move(0,3*cos(pygame.time.get_ticks()/100))
    ...
    def onkeydown(self, key, caps=None):
        if key==K_q:
            self.app.player.go_to([i * 64 for i in self.app.get_virtual_mouse_pos()],"nw")    
        elif key==K_e:
